...
module SwaggerClient
  class AuthorizationResponse
    # エンドユーザにより認可(許諾)された場合に、発行される認可コード。
    attr_accessor :code

    # 要求時に指定された値をそのまま返却する。 貴社にて要求時と同一であることを確認。
    attr_accessor :state
    ...
    def list_invalid_properties
      ...
      if @state.to_s.length > 128
        invalid_properties.push('invalid value for "state", the character length must be smaller than or equal to 128.')
      end
    ...
    def valid?
      ...
      return false if @state.to_s.length > 128
    ...
    def state=(state)
      ...
      if state.to_s.length > 128
        fail ArgumentError, 'invalid value for "state", the character length must be smaller than or equal to 128.'
      end
